...
stock2covariates <- function(file, g = c(0.95, 0.80),
                             kappa = 1, ma = c(1, 5, 20))
  {
    ...
    Returns = 100*log(Close/Close1) 
    tInit <- 2 # The initial point by taking away the first lag.
    tIdx <- (max(ma, 3)+2):nObs 

    ## Loop over all the observations and construct the covariates. NOTE: This
        ...
        for(j in 1:length(g))
          {
            ...
            MinMaxAbs[t, j] <- (1-g[j])*
              sum(g.s*(100*log(High[tInit:(t-2)]/Low[tInit:(t-2)]))^kappa)
          }
      }
    outData <- cbind(RMA, CloseAbs, CloseSqrt, MinMaxAbs)[tIdx, , drop = FALSE]
