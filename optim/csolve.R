csolve0 <- function(FUN,x,...,gradfun=NULL,crit=1e-7,itmax=20,verbose=TRUE,alpha=1e-3,delta=1e-6,long=FALSE) {
...
###           **************************************
###           Need to rewrite to take 1advantage of R's ability to do analytic derivative and function evaluation
###           jointly, automatically finding common expressions.
###           **************************************
### x:        initial value for FUN's argument. Must be a matrix with a non-NULL dimnames[[1]], which contains the variable names.
### gradfun:  the function called to evaluate the gradient matrix.  If this
###           is NULL (the default), a numerical gradient is used instead.
